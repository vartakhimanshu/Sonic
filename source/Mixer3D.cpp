...
Mixer3D::Mixer3D(int bufSize, int smpRate, int bitD, World *w)
{
	...
	testOutput = new long[18 * dataSize];
	result = new complex[dataSize];
	cbResult = new long[2*dataSize*16];
	cbResultLeft = new long[dataSize*16];
	cbResultRight = new long[dataSize*16];
...
void Mixer3D::stereoTest()
{
	...
	for (int i = 0; i < 2 * fullLength; i++)
	{
		cbResult[i] = cbResult[i] * (pow(2, 15)/max);
	}
...
template void Mixer3D::write< long >(std::ofstream& stream, const long& t);
...
template void Mixer3D::writeWAVData<long>(const char* outFile, long* buf, size_t bufSize, int sampleRate, short channels);